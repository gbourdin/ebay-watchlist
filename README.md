...
- `uv run ebay-watchlist cleanup-expired-items --retention-days 30`
- `uv run ebay-watchlist run-loop --cleanup-retention-days 30 --cleanup-interval-minutes 1440`
...
- Push to `main` (or `master`) builds and publishes `gbourdin/ebay-watchlist:latest`.
- Push tag `vX.Y.Z`:
