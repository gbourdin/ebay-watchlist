...
export default function ManagePage() {
  ...
  return (
    <section className="space-y-6">
      <header>
        <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-100">Manage Watchlist</h1>
        <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
          Configure watched sellers and category IDs used by daemon fetches.
        </p>
      </header>

      {loading && <p className="text-sm text-slate-500 dark:text-slate-400">Loading watchlist...</p>}
      ...
      {watchlist && (
        <div className="grid gap-6 xl:grid-cols-2">
          <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900 dark:shadow-none">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Watched Sellers</h2>
            ...
            <ul className="mt-4 space-y-2">
              {watchlist.sellers.map((seller) => (
                ...
                >
                  <span className="font-medium text-slate-800 dark:text-slate-100">{seller}</span>
                  <button
                    type="button"
                    disabled={busy}
              ...
              {watchlist.sellers.length === 0 && (
                <li className="rounded-lg border border-dashed border-slate-300 px-3 py-4 text-sm text-slate-500 dark:border-slate-600 dark:text-slate-400">
          ...
          <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900 dark:shadow-none">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Watched Categories</h2>
            ...
            >
              ...
              <button
                type="submit"
                disabled={busy}
                className="self-start rounded-lg bg-slate-900 px-3 py-2 text-sm font-medium text-white disabled:opacity-60 dark:bg-slate-100 dark:text-slate-900"
              >
                Add category
              </button>
            </form>
              ...
              {watchlist.categories.map((category) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400">ID: {category.id}</p>
                  </div>
                  <button
                    type="button"
                    disabled={busy}
